...
class CameraDevice:
    ...
    def __init__(self, index: int, name: str = None, backend: str = None):
        ...
        Args:
            ...
            backend: OpenCV backend name (e.g., 'V4L2', 'DSHOW')
        """
        self.index = index
    ...
    def __repr__(self) -> str:
        return f"CameraDevice(index={self.index}, name='{self.name}', backend='{self.backend}')"
...
class CameraManager:
            ...
            try:
                ...
                if cap.isOpened():
                    ...
                    if ret:
                        ...
                        device = CameraDevice(index=i, name=name, backend=backend)
    ...
    def connect(self, device_index: int = 0) -> bool:
        ...
        Args:
            device_index: Camera device index to connect to

        Returns:
            True if connection successful, False otherwise
        """
        logger.info(f"Attempting to connect to camera {device_index}...")
        ...
        try:
            ...
            if not ret:
                logger.error(f"Camera {device_index} opened but cannot read frames")
                self.cap.release()
                self.cap = None
                return False
            ...
            logger.info(
                f"Successfully connected to camera {device_index} "
            ...
            logger.error(f"Exception while connecting to camera {device_index}: {e}")
            self.disconnect()
            return False
    ...
    def disconnect(self) -> None:
        ...
        if self.cap is not None:
            try:
                self.cap.release()
                logger.info(f"Disconnected from camera {self.current_device}")
            except Exception as e:
